...
> R: carrito_init inicializa el carrito con una cantidad 0 de items. Si no se inicializa, la cantidad tampoco, por lo que no se podría evaluarla
...
> R: El bug está en la linea 24, que suma al total el precio de un tipo de item

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería sumar el precio del tipo de item, multiplicado por su cantidad
...
> R: El operador incorrecto era <=, cuando debería ser <
...
> R: Sí, las líneas 29 y 30, correspondientes a la función carrito_descuento. No se ejecutó por que la cobertura solo compiló y evaluó los tests unitarios, donde dicha función no es llamada
...
> R: Un test unitario verifica que una función trabaje bien de forma independiente. Un test de integración verifica que varias funciones trabajen bien en conjunto
...
> R: Porque en lenguajes como Python o Java hay una capa de abstracción que impide asumir que una lista tiene más elementos de los que se definieron. En cambio, en C esta capa no existe. Si se tiene una lista de de tamaño 2 y se accede a su tercera posición, se accederá a un espacio de memoria que podría estar usando otra variable
...
> R: Mejora la organización a la hora de arreglar los bugs, ya que con los tests bien definidos se puede cortar un gran margen de código que no hay que revisar. Los tests pueden determinar el alcance del programa y, bajo esa perspectiva, se le dice cómo debe responder y se analizan sus respuestas, una por una. Así, se puede usar un código de funciones pequeñas y manejables para revisar uno mucho más amplio
...
> R: Porque el primer test no evaluaba una cantidad mayor a 1, que era lo que producía el problema, ya que faltaba multiplicar por la cantidad
